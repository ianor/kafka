...
Client.prototype.init = function () {
    ...
    // parse the initial brokers looking for hosts that should be mapped to a proxy
    if (self.options.proxy) {
        self.initialBrokers = self.initialBrokers.map(function (broker) {
            var proxy = self.options.proxy[broker.host];
            var brokerHost = proxy && proxy.host ? proxy.host : broker.host;
            var brokerPort = proxy && proxy.port ? proxy.port : broker.port;
            return Object.assign(broker, { host: brokerHost, port: brokerPort });
        });
    }
...
Client.prototype.updateMetadata = function () {
    ...
    self._updateMetadata_running = (function _try() {
        ...
        return self.metadataRequest().then(function (response) {
            ...
            // allow broker host and port to be overwritten by proxy values so that
            // we can use ssh tunnels and closed subnets, etc.
            if (self.options.proxy) {
                var brokers = response.broker.map(function (broker) {
                    var proxy = self.options.proxy[broker.host];
                    var brokerHost = proxy && proxy.host ? proxy.host : broker.host;
                    var brokerPort = proxy && proxy.port ? proxy.port : broker.port;
                    return Object.assign(broker, { host: brokerHost, port: brokerPort });
